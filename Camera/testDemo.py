...
def demo(cvImage, nRet, camera, cnt):
    # if cnt == 0:
    ...
    for i in range(0, cameraCnt):
        ...
        # 设置相机参数
        nRet[i] = setExposureTime(camera[i], 4000)  # 曝光
        nRet[i] = setGamma(camera[i], 1)  # 伽马
        nRet[i] = setGainRaw(camera[i], 5)  # 增益
        nRet[i] = setBrightness(camera[i], 50)  # 亮度
    ...
    for i in range(0, cnt):
        ...
        if convertParams.pixelForamt == EPixelType.gvspPixelMono8:
            ...
            cvImage.append(numpy.array(colorByteArray).reshape(convertParams.height, convertParams.width, 3))
            # if ( nRet != 0 ):
    ...
    # if cv2.waitKey(1) == 27:
    #     break
    return cvImage, nRet, camera, cnt
